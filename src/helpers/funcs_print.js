...
function draw_en_tete(
  ...
  let poste_zh = POSTES[poste]?.zh || "";
  let poste_fr = POSTES[agent_data.poste]?.fr || "";
  if (poste_fr.trim().length > 0) poste_fr += ": ";
  ...
  const text_poste = [{ lat: poste_fr }, { lat: "ATELIER DE CIMENT" }];
...
function print_agent_roulement(doc, agent_data, print_empty, arrayPrintSim) {
  ...
  doc.addPage();
  const { matricule, month, year, nom, postnom, poste } = agent_data;
